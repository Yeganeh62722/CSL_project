...
import math
import ctypes
from ctypes import CDLL, c_float, byref, POINTER, pointer
import time
...
thumb_x = SLIDER_X + 70 # Initial position of the thumb
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_m:  # Detect "M" key press
                asm_mode = not asm_mode
    ...
    if asm_mode:
        ...
        if ball_thrown_straight:
            ...
            straight_lib.update_ball_position.argtypes = [POINTER(c_float), POINTER(c_float)]
        ...
        # Update ball position for sinusoidald throw
            sinusoidal_lib.update_sinusoidal_ball_position.argtypes = (POINTER(c_float), POINTER(c_float), POINTER(c_float), POINTER(c_float))
        ...
        if ball_thrown_sinusoidal:
            ...
            # Call the Assembly function
            sinusoidal_lib.update_sinusoidal_ball_position(pointer(ball_x), pointer(ball_y), pointer(ball_speed_x), pointer(sinusoidal_angle))
            ball_x = float(ball_x.value)
    ...
    # Mode value Label
    if asm_mode:
        mode_text = font.render(f"Mode: Assembly", True, BLUE)
    else:
        mode_text = font.render(f"Mode: Python", True, BLUE)
    screen.blit(mode_text, (SLIDER_X, SLIDER_Y + 30))
